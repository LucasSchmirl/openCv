...
int main( int, char** argv ) {
    ...
    cv::Mat camImg;                                // Image from path
    camImg = cv::imread(fname, cv::IMREAD_COLOR);
    ...
    cv::Rect ROI = cv::Rect( 800, 350, 150, 300 ); // ROI to learn object
    //----------------------------//
    ...
    while( key != 'd' ) {
        ...
        cv::resizeWindow( "Img", img_raw.size());
